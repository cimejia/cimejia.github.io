...
async function registerRoutes(app2) {
  app2.post("/api/visits/:page", async (req, res) => {
    const { page } = req.params;
    const count = await storage.incrementVisit(page);
    res.json({ count });
  });
  app2.get("/api/visits/:page", async (req, res) => {
    const { page } = req.params;
...
(async () => {
  ...
  const port = 5e3;
  const tester = createServer2().once("error", (err) => {
